...
void find_hull(const Point& start, const Point& fin, vector<Point> points, vector<Point> & hull)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        if (i != index)
        {
            if ( orientation(start, points[index], points[i]) == 2) first_set.push_back(points[i]);
            if ( orientation(fin, points[index], points[i]) == 1) second_set.push_back(points[i]);
        }
    }
...
void convexHull_R(vector<Point> points, vector<Point> & hull)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        int orient = orientation(start, finn, points[i]);
